...
├─────────────────────────────────────────────────────────────────────────────┤
│ Stage F: 双信号分支各一次预注册单向 LEFT JOIN、Baseline IC 测算与强制敏感性分析 │
...
### Stage F: 双信号分支各一次预注册单向 LEFT JOIN 与诊断评价

#### 目标
校验两个 Manifest 的跨阶段绑定哈希一致性，对 Primary 与 Zero-Excluded 双信号分支分别执行一次官方预注册单向 LEFT JOIN，输出正式 M0 基准诊断指标及全套敏感性对照。

#### 核心产物与交付
1. **双信号分支预注册 LEFT JOIN 执行与基数不变量验证**:
   ```sql
   -- Primary LEFT JOIN
   SELECT
       s.period_of_report,

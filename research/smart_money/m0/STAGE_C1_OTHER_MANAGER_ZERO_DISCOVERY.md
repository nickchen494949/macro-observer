...
**Inference from Coexistence**: The presence of both `'0'` and positive integer sequence numbers within the same accession demonstrates that filers use `'0'` as a line-specific indicator (indicating that the line does not point to a positive numbered other included manager and is attributed to the origin filer), rather than a filing-wide omission.
...
3. **Mandatory Sensitivity Branch (`ZERO_SENTINEL_EXCLUDED`)**:
   - Require a dedicated signal-side sensitivity branch derived **upstream pre-aggregation** that excludes empirical-zero (`'0'`) rows to generate a separate immutable table `m0_signals_zero_excluded`.
   - Independently LEFT JOINed to the same frozen `m0_forward_returns` outcome table to evaluate signal robustness against this compatibility rule.
...
| :--- | :--- | :--- |
| **Point72 2019Q4 Main Filing** | 917 rows in accession `0001567619-20-004063` have `INVESTMENTDISCRETION='DFND'`, `other_manager='0'`, and shared voting authority for all 418M shares. | Point72 intended to report these holdings under its own defined reporting scope rather than delegating to another numbered manager. |
| **SEC Regulatory FAQs** | SEC FAQ Q48/Q49 explicitly specifies blank or `N/A` when no other included manager number is assigned; does not authorize `0` or `NONE`. | `'0'` is an empirical electronic filing convention used when a line does not point to a positive numbered other included manager, attributable to the origin filer reporting scope. |

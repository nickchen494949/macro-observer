 ...
 *
 * Uses REAL SPX data where available (PARTIAL_SCENARIO), simulated flat for unknown.
 * Never uses LOCKED — RP has no structural lag.
 *
...
function computeRpForwardPressure(rawSpx, rawDgs10, calendar, config) {
  ...
  let lastSpxPrice = null, spxLastDate = null;
  for (const pt of rawSpx) {
    ...
    if (price != null && !isNaN(price)) {
      ...
      if (!spxLastDate || pt[0] > spxLastDate) {
        spxLastDate = pt[0];
        lastSpxPrice = price;
      }
    }
  ...
  let lastDgs10Yield = null, dgs10LastDate = null;
  for (const pt of rawDgs10) {
    if (pt[1] != null && !isNaN(pt[1])) {
      dgs10Map.set(pt[0], pt[1]);
      if (!dgs10LastDate || pt[0] > dgs10LastDate) {
        dgs10LastDate = pt[0];
        lastDgs10Yield = pt[1];
      }
    }
  ...
  if (!lastSpxPrice || !lastDgs10Yield) return { status: 'insufficient_data' };
  ...
  // SPX: use real data if available, else flat (lastSpxPrice)
  // DGS10: use real data if available, else flat (lastDgs10Yield = ΔY=0)
  const fullExtSpx = [...rawSpx];
  const fullExtDgs10 = [...rawDgs10];
  for (let d = 0; d < 5; d++) {
    const date = futureDates[d];
    if (!spxMap.has(date)) fullExtSpx.push([date, lastSpxPrice]);
    if (!dgs10Map.has(date)) fullExtDgs10.push([date, lastDgs10Yield]);
  }
  ...
  for (const h of computeHorizons) {
    ...
    if (spxKnown && bondKnown) {
      ...
      certaintySemantic = 'equity_observed_bond_assumed_flat_yield';
    } else {
      certaintyLevel = 'SCENARIO';
      certaintySemantic = 'both_legs_zero_return_path';
    }

    // Cumulative deltas vs today (T+0)
  ...
  return {
    ...
    methodology: 'Zero-shock decay path: equity return = 0, yield change = 0. Re-runs canonical buildAlignedReturns → computeRpSnapshot per horizon.',
    disclaimer: 'Reduced 2-asset RP proxy. PARTIAL_SCENARIO uses real equity data with assumed flat bond yield. SCENARIO assumes both legs flat. NOT a market forecast.'
  };
}

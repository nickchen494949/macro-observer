...
// ── GDELT Config ──────────────────────────────────────────────
const GDELT_QUERY = [
  'inflation', '"federal reserve"', '"interest rate"', 'tariff',
  'recession', '"labor market"', '"central bank"', '"treasury yield"',
  '"trade war"', '"economic growth"', '"rate cut"', '"rate hike"',
  'unemployment', '"consumer spending"', '"GDP"',
].join(' OR ');
...
async function fetchGdelt() {
  try {
    console.log('[News] Fetching GDELT macro news...');
    const { status, body } = await httpGet(GDELT_URL);
    if (status !== 200) {
    ...
    console.log(`[News] GDELT returned ${articles.length} articles`);
    ...
    console.log(`[News] GDELT fetch failed: ${err.message}`);
    return [];
  }
}

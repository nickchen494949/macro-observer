...
function runFlowEngine(config) {
  ...
  let rpMechPressure;
  if (rpStatus === 'ok' && spx && dgs10 && spx.length > 65 && dgs10.length > 65) {
    // Build aligned returns from the calendar-aligned production data
    const rpAligned = buildAlignedReturnsRP(spx, dgs10, 8);
    rpMechPressure = computeRpMechanicalPressure(rpAligned.eqReturns, rpAligned.bondReturns);
    rpMechPressure.observationDate = commonAsOfDate;

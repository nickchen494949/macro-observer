...
function runFlowEngine(config) {
  ...
  if (vcMechPressure.status === 'ok' && spx && spx.length > 45) {
    ...
    for (const h of fwdHorizons) {
      // Observation index for T+h: (spx.length - 1 - 2) + h = currentObsIdx + h
      const futureObsIdx = currentObsIdx + h;
      const isLocked = h <= 2; // T+1/T+2 use only known data
      ...
      let pressureDirection = 'neutral';
      if (deltaFromCurrent != null) {
        if (deltaFromCurrent > 0.005) pressureDirection = 'buying';
        else if (deltaFromCurrent < -0.005) pressureDirection = 'selling';
      }

      // Analyze roll-off for the binding window at the future horizon
      const bindingWindowSize = fwdBindingWindow === '40D' ? 40 : 20;
      const rollOff = analyzeRollOff(currentObsIdx, futureObsIdx, bindingWindowSize);

      // Also analyze the non-binding window for context
      const otherWindowSize = fwdBindingWindow === '40D' ? 20 : 40;
      const otherRollOff = analyzeRollOff(currentObsIdx, futureObsIdx, otherWindowSize);

      // Build explanation
      let explanation = null;
      if (rollOff.primaryRollOff && deltaFromCurrent != null && Math.abs(deltaFromCurrent) > 0.001) {
        const absRet = Math.abs(rollOff.primaryRollOff.returnPct);
        const retSign = rollOff.primaryRollOff.returnPct < 0 ? 'negative' : 'positive';
        const volEffect = absRet > 0.5 ? 'vol_decreasing' : 'vol_minimal_change';
        const exposureEffect = deltaFromCurrent > 0 ? 'exposure_rising' : 'exposure_falling';
        explanation = `${retSign} ${absRet.toFixed(1)}% return from ${rollOff.primaryRollOff.date} exits ${fwdBindingWindow} window → ${volEffect} → ${exposureEffect}`;
        rollOff.primaryRollOff.volEffect = volEffect;
      }
      ...
      horizonResults.push({
        ...
        pressureDirection,
        primaryRollOff: rollOff.primaryRollOff,
        allDroppedReturns: rollOff.droppedReturns,
        explanation
      });
    }

    // Determine dominant forward signal from LOCKED horizons first, then SCENARIO
    let dominantSignal = 'neutral_under_zero_return_path';
    const lockedHorizons = horizonResults.filter(h => h.certaintyLevel === 'LOCKED' && h.deltaFromCurrent != null);
    const scenarioHorizons = horizonResults.filter(h => h.certaintyLevel === 'SCENARIO' && h.deltaFromCurrent != null);
    const allValid = [...lockedHorizons, ...scenarioHorizons];

    if (allValid.length > 0) {
      const avgDelta = allValid.reduce((s, h) => s + h.deltaFromCurrent, 0) / allValid.length;
      if (avgDelta > 0.005) dominantSignal = 'mechanical_buying_under_zero_return_path';
      else if (avgDelta < -0.005) dominantSignal = 'mechanical_selling_under_zero_return_path';
    }

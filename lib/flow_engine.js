...
function runFlowEngine(config) {
  ...
  // 4b. RP Mechanical Pressure — Target-vol leverage layer
  // ASSUMPTIONS (not verified against industry data):
  const RP_TARGET_VOL = 0.10;   // 10% annualized portfolio vol target
  const RP_LEVERAGE_CAP = 3.0;  // max leverage
  const RP_LEVERAGE_FLOOR = 0.2;// min leverage

  let rpMechPressure;
  if (rpStatus === 'ok' && estEqAlloc != null && eqVol20d != null && bondVol20d != null && stockBondCorr60d != null) {
    // Portfolio variance: σ_p² = w_eq²·σ_eq² + w_bond²·σ_bond² + 2·w_eq·w_bond·ρ·σ_eq·σ_bond
    const portVar = estEqAlloc**2 * eqVol20d**2
                  + estBondAlloc**2 * bondVol20d**2
                  + 2 * estEqAlloc * estBondAlloc * stockBondCorr60d * eqVol20d * bondVol20d;
    const portVol = Math.sqrt(Math.max(portVar, 0));

    // Target-vol leverage
    let targetLev = portVol > 0 ? RP_TARGET_VOL / portVol : 1;
    targetLev = Math.min(targetLev, RP_LEVERAGE_CAP);
    targetLev = Math.max(targetLev, RP_LEVERAGE_FLOOR);

    // Gross exposures
    const eqGross = estEqAlloc * targetLev;
    const bondGross = estBondAlloc * targetLev;

    // 5-day-ago leverage (using 5d-ago weights and vol)
    let targetLev_5d = null;
    let eqGross_5d = null, bondGross_5d = null;
    if (estEqAlloc_5d != null && eqVol20d_5d != null && bondVol20d_5d != null) {
      const estBondAlloc_5d = 1 - estEqAlloc_5d;
      // 5d-ago correlation: use aligned returns ending 5 days earlier
      const aligned60_5d = getAlignedReturns(
        spx ? spx.slice(0, spx.length - 5) : null,
        dgs10 ? dgs10.slice(0, dgs10.length - 5) : null,
        60,
        (curr, prev) => Math.log(curr/prev),
        (curr, prev) => -8 * (curr - prev) / 100
      );
      const corr_5d = calcCorr(aligned60_5d.ret1, aligned60_5d.ret2);
      const usableCorr5d = corr_5d != null ? corr_5d : 0;

      const portVar_5d = estEqAlloc_5d**2 * eqVol20d_5d**2
                       + estBondAlloc_5d**2 * bondVol20d_5d**2
                       + 2 * estEqAlloc_5d * estBondAlloc_5d * usableCorr5d * eqVol20d_5d * bondVol20d_5d;
      const portVol_5d = Math.sqrt(Math.max(portVar_5d, 0));
      targetLev_5d = portVol_5d > 0 ? RP_TARGET_VOL / portVol_5d : 1;
      targetLev_5d = Math.min(targetLev_5d, RP_LEVERAGE_CAP);
      targetLev_5d = Math.max(targetLev_5d, RP_LEVERAGE_FLOOR);
      eqGross_5d = estEqAlloc_5d * targetLev_5d;
      bondGross_5d = estBondAlloc_5d * targetLev_5d;
    }

    const leverageChange5d = targetLev_5d != null ? targetLev - targetLev_5d : null;
    const eqExpChange5d = eqGross_5d != null ? eqGross - eqGross_5d : null;
    const bondExpChange5d = bondGross_5d != null ? bondGross - bondGross_5d : null;

    // Pressure direction (based on leverage change)
    let pressureDir = 'neutral';
    if (leverageChange5d != null) {
      if (leverageChange5d < -0.01) pressureDir = 'deleveraging';
      else if (leverageChange5d > 0.01) pressureDir = 'leveraging';
    }

    // Broad deleveraging: BOTH stock AND bond gross exposures decreased
    const broadDelev = (eqExpChange5d != null && bondExpChange5d != null
                       && eqExpChange5d < -0.005 && bondExpChange5d < -0.005);

    rpMechPressure = {
      status: 'ok',
      equityWeight: Number(estEqAlloc.toFixed(4)),
      bondWeight: Number(estBondAlloc.toFixed(4)),
      portfolioVol: Number(portVol.toFixed(4)),
      stockBondCorrelation: Number(stockBondCorr60d.toFixed(4)),
      targetLeverage: Number(targetLev.toFixed(4)),
      leverageChange5d: leverageChange5d != null ? Number(leverageChange5d.toFixed(4)) : null,
      equityGrossExposure: Number(eqGross.toFixed(4)),
      bondGrossExposure: Number(bondGross.toFixed(4)),
      equityExposureChange5d: eqExpChange5d != null ? Number(eqExpChange5d.toFixed(4)) : null,
      bondExposureChange5d: bondExpChange5d != null ? Number(bondExpChange5d.toFixed(4)) : null,
      pressureDirection: pressureDir,
      broadDeleveraging: broadDelev,
      observationDate: commonAsOfDate,
      assumptions: {
        targetPortfolioVol: RP_TARGET_VOL,
        leverageCap: RP_LEVERAGE_CAP,
        volLookback: 20,
        corrLookback: 60,
        bondDuration: 8,
        label: 'ASSUMPTION'
    ...
    rpMechPressure = {
      ...
      stockBondCorrelation: null, targetLeverage: null, leverageChange5d: null,
      equityGrossExposure: null, bondGrossExposure: null,
      equityExposureChange5d: null, bondExposureChange5d: null,
      pressureDirection: null, broadDeleveraging: null,
      observationDate: null,
      assumptions: { targetPortfolioVol: RP_TARGET_VOL, leverageCap: RP_LEVERAGE_CAP, volLookback: 20, corrLookback: 60, bondDuration: 8, label: 'ASSUMPTION' },
      disclaimer: 'Reduced 2-asset RP proxy. Does not represent actual RP fund positions or AUM.'
    };
  }

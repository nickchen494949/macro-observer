...
function runFlowEngine(config) {
  ...
  if (vcMechPressure.status === 'ok' && spx && spx.length > 45) {
    ...
    const fwdHorizons = [1, 2, 3, 4, 5]; // Compute all 5 so each day's daily delta is vs previous day
    const displayHorizons = new Set([1, 2, 3, 5]); // Only show these in API output
    ...
    // Filter to display horizons only (T+4 was computed but not shown)
    const displayResults = horizonResults.filter(h => displayHorizons.has(h.horizon));
  ...
  const { computeRpMechanicalPressure, computeRpForwardPressure, buildAlignedReturns: buildAlignedReturnsRP } = require('./rp_mechanical');
  ...
  // 4c. RP Forward Mechanical Pressure — Zero-Shock Decay Path
  let rpForwardPressure = { status: 'insufficient_data' };
  if (rpMechPressure && rpMechPressure.status === 'ok' && rawSpx && rawDgs10 && usEquityCalendar.length > 0) {
    try {
      rpForwardPressure = computeRpForwardPressure(rawSpx, rawDgs10, usEquityCalendar);
    } catch (e) {
      rpForwardPressure = { status: 'error', error: e.message };
    }
  }
  ...
  const summary = {
    ...
    narrative: {
      ...
      zh: `代理模型识别${activeFlowMechanismCount}个活跃的前瞻资金流机制和${activeRotationMechanismCount}个近期轮动机制，${dominantRegime === 'conflicting' ? '资金流方向互相冲突' : dominantRegime === 'procyclical_buy' ? '整体资金压力偏向买入' : dominantRegime === 'procyclical_sell' ? '整体资金压力偏向卖出' : dominantRegime === 'no_dominant_flow' ? '无明显主导资金流方向' : '资金流方向未定'}。`
    }
  };

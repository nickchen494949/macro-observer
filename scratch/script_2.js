    ...
    function updateFreshness(idPrefix, data, viewState) {
      if (data?.commonAsOfDate) {
        ...
        if(globalAsOfDate === '--' || data.commonAsOfDate > globalAsOfDate) {
          ...
          if (viewState.apiStatus === 'current' && viewState.marketFreshness === 'current' && viewState.quality === 'complete') {
    ...
    function renderAll(data, apiConnectionFailed = false, isPartialOverlay = false) {
      removeSkeletons();
    ...
    window.fetchFlows = async function() {
    ...
    (async function() {
      ...
      await window.fetchFlows();
      setInterval(window.fetchFlows, 60000);
    })();
